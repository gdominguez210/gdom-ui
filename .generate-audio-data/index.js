...
export function getInterpolatedValueCubic(data, exactIndex) {
...
export function getInterpolatedPeakCubic(data, exactIndex, options = {}) {
 ...
 */
function generatePeaksForZoomLevel(rawAudioData, samplesPerPixel, normalize = true) {
    ...
    let maxAbsValue = 0;
    console.log(`Generating ${samplesPerPixel} SPP level (${useCubicInterpolation ? 'cubic' : 'envelope'})...`);
...
const argv = yargs(hideBin(process.argv))
    ...
    .middleware((argv) => {
    if (!argv.output)
        argv.output = getOutputPath(argv.input);
})
    .help()
    .example('$0 --input audio/example.wav', 'Generate waveform with default settings')
    .example('$0 -i audio/example.wav --normalize false', 'Generate waveform without normalization').argv;
async function generateWaveformData() {
    try {
        const { normalize, input, output } = argv;
  ...
  `);
        ...
        const audioData = await import('audio-decode').then(({ default: decode }) => decode(readFileSync(input)));
