...
const useKeyboardMediaSeek = require('./useKeyboardMediaSeek-CglZ2tDM.js');
const useDelayedMouseMove = require('./useDelayedMouseMove-C5F20XFY.js');
const useMousePositionRef = require('./useMousePositionRef-CeslLEtk.js');
const useElementDimensions = require('./useElementDimensions-1QpEcy1U.js');
...
function useAudioWaveformProgress(props) {
  ...
  const { elementRef, getElementDimensions } = useElementDimensions.useElementDimensions();
  const {
    getIsHovering,
    getMousePosition,
  ...
  const { canvasRef, drawWaveform, handleResize } = useAudioWaveformEnvelopeRectangles.useAudioWaveformEnvelopeRectangles({
    data,
    color: colorFn,
    gapMaxWidth,
    gapMinWidth,
  ...
  const mergedRef = useComposedRefs.useComposedRefs(elementRef, canvasRef);
