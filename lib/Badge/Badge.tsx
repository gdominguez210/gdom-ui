...
function _Badge(props: BadgeProps, ref: Ref<HTMLElement>){
  const { as = 'span', children, className, variant = 'neutral', size = 'md', ...restProps } = props;

  const Node = as;

  return <Node className={
    twMerge(cn(styles.base, styles[variant], styles[size], className))
  } {...restProps} ref={ref}>{children}</Node>;
};
